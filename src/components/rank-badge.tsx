...
export function RankBadge({ rank, dept = "Acting" }: { rank: number; dept?: TopRankDept }) {
  ...
  return (
    <span
      ...
      title={`${label} · #${rank}`}
      className="absolute left-2 top-2 flex cursor-pointer items-center gap-0.5 rounded-md border border-edge-soft/60 bg-canvas/95 px-1.5 py-0.5 text-[10.5px] font-bold text-ink transition-all hover:scale-105 hover:border-accent/60 hover:bg-canvas"
    >
