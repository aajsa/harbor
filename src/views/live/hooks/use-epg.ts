...
export function useEpg(source: IptvPlaylistSource | null): {
  index: EpgIndex | null;
  loading: boolean;
  error: string | null;
} {
  ...
  useEffect(() => {
    ...
    const urls = source.epgUrl ? [source.epgUrl] : deriveEpgUrls(source.url);
    if (urls.length === 0) {
      setIndex(null);
      return;
    }
  ...
  }, [source?.id, source?.url, source?.epgUrl]);
